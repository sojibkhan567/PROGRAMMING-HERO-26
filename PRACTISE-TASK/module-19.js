...
let productList = [{ name: "Pen", stock: 5 }, { name: "Bag", stock: 0 }, {
    name: "Notebook", stock: 2
}];
//console.log(getAvailableProducts(productList))
...
const getTotalStockValue = (products) => {
    const total = products.reduce((price, product) => price + (product.price * product.stock), 0)
    return total;
}
...
const getDiscountedTotalForCategory = (products, category) => {
    let totalPrice = products.filter((p) => p.category === category)
        .map((p) => p.price - (p.price * (10 / 100)))
        .reduce((total, price) => total + price, 0);
    return totalPrice;
}
let productsCategory = [{ name: "Pen", category: "stationery", price: 100 }, {
    name: "Bag",
...
//console.log(getDiscountedTotalForCategory(productsCategory, "stationery"));
